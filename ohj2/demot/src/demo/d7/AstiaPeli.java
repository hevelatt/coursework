...
public class AstiaPeli {
    ...
    public void tulostaOhje() {
        if ( astiat.size() <= 0 ) return;
        String ja = "";
        System.out.print("Käytössäsi on ");
        for (int i = 1; i < astiat.size(); i++) {
    ...
    /**
     * @return Palauttaa astioiden (ei ämpäri) nestemäärän summan
     * AstiaPeli peli = new AstiaPeli();
     * peli.lisaaAstia("5",5);
     * peli.lisaaAstia("8",8);
     * peli.astioidenSumma() ~~~ 13;
     */
    public double astioidenSumma() {
        double summa = 0;
        for (int i = 0; i < astiat.size(); i++)
            summa += anna(i).getMaara();
        return summa;
    }
